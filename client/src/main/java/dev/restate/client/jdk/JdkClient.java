...
//  }
    ...
    return new AwakeableHandle() {
      ...
      public <T> CompletableFuture<Void> resolveAsync(
        ...
        HttpRequest request =
            reqBuilder
                .POST(
                    HttpRequest.BodyPublishers.ofByteArray(serde.serialize(payload).toByteArray()))
                .build();
        return httpClient
            .sendAsync(request, HttpResponse.BodyHandlers.ofByteArray())
  ...
  private <Req> HttpRequest prepareHttpRequest(
    ...
    return reqBuilder
        .POST(HttpRequest.BodyPublishers.ofByteArray(reqSerde.serialize(req).toByteArray()))
        .build();
  }
  ...
  static IngressException createIngressException(
      String message, HttpRequest request, Throwable cause) {
    return new IngressException(
        message, request.method(), request.uri().toString(), -1, null, cause);
  }
  ...
  static IngressException createIngressException(
      String message, HttpResponse<byte[]> response, Throwable cause) {
    return new IngressException(
        message,
