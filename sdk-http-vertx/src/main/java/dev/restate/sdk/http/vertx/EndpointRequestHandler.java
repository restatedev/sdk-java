...
class EndpointRequestHandler implements Handler<HttpServerRequest> {
  ...
  EndpointRequestHandler(EndpointImpl endpointImpl, OpenTelemetry openTelemetry) {
  ...
  private void handleDiscoveryRequest(HttpServerRequest request) {
    ...
    try {
      discoveryResponse = endpointImpl.handleDiscoveryRequest(request.getHeader(ACCEPT));
    } catch (ProtocolException e) {
  ...
  public static EndpointRequestHandler fromEndpoint(Endpoint endpoint) {
