...
public class RestateHttpServer {
    ...
    private static int DEFAULT_PORT = Optional.ofNullable(System.getenv("PORT")).map(Integer::parseInt).orElse(9080);
private   static HttpServerOptions DEFAULT_OPTIONS =
    ...
    /**
     * Start serving the provided {@code endpoint} on the port specified by the environment variable {@code PORT}, or alternatively on the default {@code 9080} port.
     *
     * <p>NOTE: this method will block for opening the socket and reserving the port. If you need a
     * non-blocking variant, manually create the server with {@link #fromEndpoint(Endpoint)} and start listening it.
     *
    ...
    /**
     * Create a Vert.x {@link HttpServer} from the provided endpoint, with the given {@link HttpServerOptions}.
     */
