...
final class AsyncResultsState {
    ...
    public void enqueue(Protocol.NotificationTemplate notification) {
        var notificationId = switch (notification.getIdCase()) {
            case COMPLETION_ID ->
                    new NotificationId.CompletionId(notification.getCompletionId());
        ...
        var notificationValue = switch (notification.getResultCase()) {
            case VOID -> NotificationValue.Empty.INSTANCE;
            case VALUE -> new NotificationValue.Success(Util.byteStringToSlice(notification.getValue().getContent()));
            case FAILURE -> new NotificationValue.Failure(Util.toRestateException(notification.getFailure()));
            case INVOCATION_ID -> new NotificationValue.InvocationId(notification.getInvocationId());
            case STATE_KEYS -> new NotificationValue.StateKeys(notification.getStateKeys().getKeysList().stream().map(ByteString::toStringUtf8).toList());
