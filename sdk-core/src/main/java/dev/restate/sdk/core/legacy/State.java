...
sealed interface State
  ...
  default void onNewMessage(InvocationInput invocationInput, StateContext stateContext) {
    throw ProtocolException.badState(this);
  }

  default DoProgressResponse doProgress(List<Integer> anyHandle, StateContext stateContext) {
    throw ProtocolException.badState(this);
  }

  /**
   * Internal result of the pure-Java state machine's {@code doProgress} routine. {@link
   * LegacyStateMachine} maps these to {@link StateMachine.AwaitResult}.
   */
  ...
  default Optional<StateMachine.NotificationValue> takeNotification(
  ...
  default RunResultHandle processRunCommand(String name, StateContext stateContext) {
    throw ProtocolException.badState(this);
  }
  ...
  StateMachine.InvocationState getInvocationState();
}
