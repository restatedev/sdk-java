...
import static dev.restate.sdk.core.legacy.Util.durationMin;
...
final class ProcessingState implements State {
  ...
  public RunResultHandle processRunCommand(String name, StateContext stateContext) {
    ...
    return new RunResultHandle(false, notificationHandle);
  }
  ...
  public int processStateGetCommand(String key, StateContext stateContext) {
    ...
    if (eagerStateQuery instanceof StateMachine.NotificationValue.Success) {
      commandMessageBuilder.setValue(
          Protocol.Value.newBuilder()
              .setContent(
                  sliceToByteString(
                      ((StateMachine.NotificationValue.Success) eagerStateQuery).slice()))
              .build());
    } else {
  ...
  public StateMachine.InvocationState getInvocationState() {
    return StateMachine.InvocationState.PROCESSING;
  }
}
