...
import static dev.restate.sdk.core.ExceptionUtils.nioBufferToProtobufBuffer;

import com.google.protobuf.ByteString;
import dev.restate.generated.service.protocol.Protocol;
import dev.restate.sdk.core.AsyncResults.SingleAsyncResultInternal;
import dev.restate.sdk.core.ExceptionUtils;
import dev.restate.sdk.core.HandlerContextInternal;
...
public final class HandlerContextImpl implements HandlerContextInternal {
  ...
  public void writeOutput(TerminalException throwable, SyscallCallback<Void> callback) {
    wrapAndPropagateExceptions(
        () -> {
          ...
          this.writeOutput(
              Protocol.OutputEntryMessage.newBuilder()
                  .setFailure(ExceptionUtils.toProtocolFailure(throwable))
                  .build(),
              callback);
        },
        callback);
  ...
  public void rejectPromise(
      String key, String reason, SyscallCallback<AsyncResult<Void>> callback) {
    wrapAndPropagateExceptions(
        () -> {
          LOG.trace("resolvePromise");
          this.stateMachine.processCompletableJournalEntry(
  ...
  public <T> void resolveDeferred(
      AsyncResult<T> asyncResultToResolve, SyscallCallback<Void> callback) {
    wrapAndPropagateExceptions(
