        ...
        return Stream.of(
                testInvocation(new SleepGreeter(), GreeterGrpc.getGreetMethod())
                        ...
                        .assertingOutput(messageLites -> {
                            ...
                            assertThat(msg.getWakeUpTime()).isLessThanOrEqualTo(Instant.now().toEpochMilli() + 1000);
                ...
                testInvocation(new SleepGreeter(), GreeterGrpc.getGreetMethod())
                        .withInput(
                                startMessage(2),
                                inputMessage(GreetingRequest.newBuilder().setName("Till")),
                                Protocol.SleepEntryMessage.newBuilder().setWakeUpTime(Instant.now().toEpochMilli()).build())
                        .usingAllThreadingModels()
                        .expectingOutput(
                                outputMessage(GreetingResponse.newBuilder().setMessage("Hello").build())
                        )
                        .named("Sleep 1000 ms completed"),
                testInvocation(new SleepGreeter(), GreeterGrpc.getGreetMethod())
                        .withInput(
                                startMessage(1),
                                inputMessage(GreetingRequest.newBuilder().setName("Till")),
                        ...
                        .assertingOutput(messageLites -> {
                            ...
                            assertThat(messageLites.get(2)).isEqualTo(
                                    outputMessage(GreetingResponse.newBuilder().setMessage("Hello").build()));
                        })
                        .named("Sleep 1000 ms with later completion")
        );
    }
}
